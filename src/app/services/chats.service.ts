...
export class ChatsService {
  ...
  bloquearChat( id_chat:number, id_usuario:number){
    return this.http.get(`${this.url}chats/bloquearChat.php?id_chat=${id_chat}&id_usuario=${id_usuario}`)//.pipe(retry(3))
  }
  ...
  confirmarCompra(chat:any){
    const CHAT = serialize(chat);
    return this.http.post(`${this.url}chats/confirmarCompra.php`, CHAT)//.pipe(retry(3))
  }
